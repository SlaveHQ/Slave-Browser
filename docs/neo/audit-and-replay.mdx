...
keywords: ["Slave Agent neo audit", "AI agent replay", "browser session recording", "local audit trail"]
---

Slave Agent neo records everything your AI does in the browser so you can go back and see exactly what happened. All of it lives on your computer. Nothing goes to any server we control.
...
<Frame caption="Every agent session, searchable and filterable.">
  <img src="/images/browserclaw--audit-list.png" alt="Slave Agent neo audit page showing a searchable list of agent sessions grouped by day. Columns visible: agent, title, tools used, action count, duration, and when it happened. Filter dropdowns at the top: Agent, Status, Site." />
</Frame>
...
<Frame caption="A single session, with every action laid out.">
  <img src="/images/browserclaw--task-detail.png" alt="Slave Agent neo task detail page showing a header with the agent name, status, and started time, followed by a tab strip listing every page the agent used. Below, a timeline of tool calls with arguments, results, and screenshots for each step." />
</Frame>
...
<Frame caption="Scrub through the recording. Jump to any moment.">
  <img src="/images/browserclaw--replay-scrubber.png" alt="Slave Agent neo replay screen showing a full-fidelity playback of a page the AI browsed. A play, pause, and seek scrubber sits at the bottom. On the right, an action timeline lists every step the AI took, synced to the current moment in the video." />
</Frame>
...
The Slave Agent neo server that reads and writes these files only accepts requests from your own machine. It binds `127.0.0.1`, the local loopback, so nothing on your network can reach it. No account. No telemetry. No cloud sync.
