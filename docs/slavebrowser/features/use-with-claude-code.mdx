...
Slave Agent is the best browser for AI coding agents. It comes with a built-in MCP server that gives your AI agent **full browser control** and **direct access to 40+ external services** — Gmail, Slack, GitHub, Google Calendar, Linear, Notion, and more — all through a single MCP connection.

<Note>
Unlike Chrome DevTools MCP which requires setting up debug profiles and running separate servers, Slave Agent MCP works out of the box. Just copy the URL from settings and connect.
</Note>

## Why Use Slave Agent with Claude Code?
...
<CardGroup cols={2}>
  ...
  <Card title="Zero Config Auth" icon="lock">
    Connect external services via OAuth — credentials are managed securely, never stored in Slave Agent.
  </Card>
</CardGroup>

<Tip>
Wondering how Slave Agent MCP compares to Chrome DevTools MCP or other browser automation tools? See our [detailed feature comparison](/slave agent/comparisons/chrome-devtools-mcp) covering 53 browser tools, 40+ app integrations, and why Slave Agent MCP gives developers more out of the box.
</Tip>

## Getting Started

<Steps>
  <Step title="Open Slave Agent Settings">
    Navigate to `chrome://slave agent/mcp` or click **Settings** → **Slave Agent as MCP** in the sidebar.
  </Step>
  <Step title="Copy the MCP URL">
    Copy the Server URL shown on the page (e.g., `http://127.0.0.1:9239/mcp`).

    <img src="/images/features--slave agent-mcp-settings.png" alt="Slave Agent MCP settings page showing Server URL" />
  </Step>
...
<Tabs>
  <Tab title="Claude Code">
    Add Slave Agent to Claude Code:
    ```bash
    claude mcp add --transport http slave agent <mcp_url> --scope user
    # Example: claude mcp add --transport http slave agent http://127.0.0.1:9239/mcp --scope user
    ```

    Start Claude Code and try it:
    ```bash
    claude
    > Open amazon.com in Slave Agent
    ```
    ...
    To remove later:
    ```bash
    claude mcp remove slave agent --scope user
    ```
  </Tab>
  <Tab title="Gemini CLI">
    Add Slave Agent to Gemini CLI:
    ```bash
  ...
  <Tab title="Codex">
    Add Slave Agent to OpenAI Codex CLI:
    ```bash
    codex mcp add slave agent <mcp_url> --transport http
    # Example: codex mcp add slave agent http://127.0.0.1:9239/mcp --transport http
    ```
  </Tab>
  <Tab title="OpenClaw">
    Add Slave Agent to [OpenClaw](https://openclaw.ai/) by adding it to the `mcpServers` section in your `openclaw.json` config file:
    ```json
    {
      "mcpServers": {
        "slave agent": {
    ...
    Replace the URL with your Slave Agent MCP URL from settings.
  </Tab>
  <Tab title="Claude Desktop">
    ...
    <Warning>
    Do not add Slave Agent to `claude_desktop_config.json` as a raw `{ "url": "..." }` entry. Claude Desktop local config expects a command wrapper here, so use `npx mcp-remote <mcp_url>`.
    </Warning>
...
The Slave Agent MCP server only listens on `127.0.0.1`, so by default only programs on the same computer can reach it. If your agent runs somewhere else — for example a [Hermes](https://github.com/NousResearch) or OpenClaw agent on a VPS controlling Slave Agent on your laptop — you need a reverse tunnel that forwards requests from the remote machine to the MCP server on your laptop.

<Warning>
Anyone who can reach this URL has full control of your browser, including every account you are signed into. Prefer a private tunnel like Tailscale over a public URL, and treat public tunnel URLs like passwords.
</Warning>

All commands below use port `9239` — replace it with the port shown on your `chrome://slave agent/mcp` settings page.

<Tabs>
  <Tab title="Tailscale (recommended)">
    Tailscale puts your laptop and VPS on a private network, so the MCP server is never exposed to the public internet and the URL never changes.

    1. Install [Tailscale](https://tailscale.com/download) on both your laptop and the VPS, and sign in to the same tailnet on both.
    2. On the laptop, share the MCP port with your tailnet:
    ```bash
    tailscale serve --bg 9239
    ```
    3. Tailscale prints an HTTPS URL like `https://your-laptop.tailnet-name.ts.net`. On the VPS, point your agent at that URL with the `/mcp` path:
    ```bash
    claude mcp add --transport http slave agent https://your-laptop.tailnet-name.ts.net/mcp --scope user
    ```

    Run `tailscale serve reset` on the laptop to stop sharing.
  </Tab>
  <Tab title="Cloudflare Tunnel">
    On the laptop, start a quick tunnel:
    ```bash
    cloudflared tunnel --url http://127.0.0.1:9239
    ```

    Cloudflare prints a URL like `https://random-words.trycloudflare.com`. On the VPS, connect your agent to `https://random-words.trycloudflare.com/mcp`.

    <Warning>
    Quick tunnels are public and unauthenticated — anyone who discovers the URL can control your browser. The URL also changes every time `cloudflared` restarts, so you must update your agent config after each restart. For anything long-running, use a [named tunnel](https://developers.cloudflare.com/cloudflare-one/connections/connect-networks/) with Cloudflare Access in front, or use Tailscale instead.
    </Warning>
  </Tab>
  <Tab title="ngrok">
    On the laptop, start a tunnel:
    ```bash
    ngrok http 9239
    ```

    ngrok prints a URL like `https://abc123.ngrok-free.app`. On the VPS, connect your agent to `https://abc123.ngrok-free.app/mcp`.

    <Warning>
    The URL is public — anyone who discovers it can control your browser — and it changes every time ngrok restarts unless you use a reserved domain. Use Tailscale instead for anything long-running.
    </Warning>
  </Tab>
</Tabs>

<Note>
The connection only works while your laptop is awake, Slave Agent is running, and the tunnel process is alive. If a remote agent that was working suddenly stops connecting, check those three first — and if you use ngrok or a Cloudflare quick tunnel, remember the URL rotates on every restart, so your agent may be pointing at a dead URL. Tailscale avoids this because its hostnames are stable.
</Note>
...
<Accordion title="Try these prompts once connected" icon="message">
  ...
  ```
  Open my LinkedIn profile in Slave Agent and extract my work experience as JSON
  ```

  Test your web app end-to-end without leaving the terminal — Claude navigates, interacts, and reports errors back.
  ```
  In Slave Agent, go to localhost:3000, click the login button, and check for console errors
  ```

  Capture visual snapshots of any page for debugging, documentation, or design review.
  ```
  Take a screenshot of the current page in Slave Agent and save it to screenshots/
  ```

  Search through your real browsing history to find pages you visited earlier.
  ```
  Search my Slave Agent history for "invoice" and list the recent matches
  ```
...
Slave Agent exposes **53 browser automation tools** to MCP clients, organized into the following categories:
...
Slave Agent connects your AI agent directly to the tools you already use — no separate MCP servers to install or configure. Everything is accessible through the same Slave Agent MCP connection.

### How It Works

<Steps>
  <Step title="Agent calls an external service tool">
    Your AI agent calls a tool like `gmail_search_messages` through the Slave Agent MCP.
  </Step>
  <Step title="OAuth login (first time only)">
    If this is your first time using that service, Slave Agent opens an OAuth login page in the browser. Log in and authorize access.
  </Step>
...
<Note>
Your credentials are managed securely via OAuth and are **never stored in Slave Agent**. Tokens are refreshed transparently, and you can revoke access at any time from the service provider.
</Note>
