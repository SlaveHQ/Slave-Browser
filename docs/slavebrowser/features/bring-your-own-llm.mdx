...
description: "Connect your own AI models to Slave Agent"
---

Slave Agent includes a default AI model you can use right away, but it has strict rate limits. For the best experience, bring your own API keys or run models locally.
...
Already paying for ChatGPT Pro or GitHub Copilot? Connect your existing account to Slave Agent with a single sign-in — no API keys, no extra cost.

<CardGroup cols={2}>
  <Card href="/slave agent/features/chatgpt-pro-oauth">
  ...
  <Card href="/slave agent/features/github-copilot-oauth">
...
{/* <img src="/images/moonshot-partnership-banner.png" alt="Slave Agent x Moonshot AI" className="rounded-xl" /> */}

Slave Agent has partnered with [Moonshot AI](https://www.kimi.com) to bring **Kimi K2.5** as a first-class provider. Kimi K2.5 is now the **recommended model** in Slave Agent and is set as the default provider.

For a limited time, Slave Agent users get **extended usage limits** powered by Kimi K2.5. This means you can use the AI agent, chat, and other AI-powered features with increased limits at no cost.
...
**Add to Slave Agent:**
1. Go to `chrome://slave agent/settings`
...
<Card title="Local Model Guide" icon="server" href="/slave agent/features/local-models">
