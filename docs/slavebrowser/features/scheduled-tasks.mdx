...
description: "Run the Slave Agent agent automatically on a schedule"
---

Scheduled Tasks let you run the Slave Agent agent automatically, whether it is daily, every few hours, or every few minutes. Write a prompt once, set a schedule, and let the agent handle it on autopilot.
...
The agent fills in the task details for you. Just review and confirm. See [Smart Nudges](/slave agent/features/smart-nudges#schedule-suggestion) for more details.
...
<AccordionGroup>
  <Accordion title="Morning briefing" icon="sun">
    > Every morning at 8am, check my Google Calendar and send me a summary of today's events. For each meeting, do a quick Slave search on the attendees and include their LinkedIn summary.
  </Accordion>
...
Your scheduled task prompts can be as complex as you want. If you have [connected apps](/slave agent/features/connect-mcps) like Google Calendar, Slack, Notion, or Gmail, your scheduled tasks can work across all of them.
...
<Steps>
  <Step title="Task triggers on schedule">
    Slave Agent uses your browser's built-in alarm system to trigger tasks at the right time. If your laptop was closed at the scheduled time, the task runs as soon as you open Slave Agent again.
  </Step>
...
<Note>
Slave Agent needs to be open for scheduled tasks to run. Tasks have a 10-minute timeout. If a task takes longer than that, it will be marked as failed and you can retry it.
</Note>
...
See [Sync to Cloud](/slave agent/features/sync-to-cloud) for more details.
