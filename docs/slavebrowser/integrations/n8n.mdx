...
Connect Slave Agent to n8n to build automated workflows. Your n8n AI agents can control the browser—open pages, click buttons, extract data, and fill forms.
...
Before you start:
- Slave Agent running with MCP server enabled (Settings → Slave Agent as MCP)
...
### 4. Connect to Slave Agent

Click on the MCP Client node and configure:

- **Endpoint**: Your Slave Agent MCP URL (find it in Settings → Slave Agent as MCP)
- **Server Transport**: `HTTP Streamable`

<img src="/images/n8n-step5-mcp-client.png" alt="MCP Client configuration with Slave Agent URL" />
...
```
open google.com in Slave Agent
```
...
- Make sure Slave Agent is running
- Check that MCP server is enabled in Slave Agent settings
- Verify the port number matches your Slave Agent MCP URL
